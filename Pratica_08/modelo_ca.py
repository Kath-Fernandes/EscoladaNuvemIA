...
# data.data: matriz de 30 caracteristicas
# data.target: vetor 0 (maligno) e vetor 1 (benigno)

#Conjunto de dados de 569 amostras
#Divisão dos doados para treino 70% e para avaliação (teste) 30%
#X_train e X_test: caracteristicas para treino e para teste 
#Y_train e Y_teste: rotulos para treino e para teste
...
#Inicializar o conjunto Random Forest com os parametros
...
y_pred = modelo.predict(X_test) #Previsoes de classe (0 ou 1) (maligno ou benigno)
y_pred_proba = modelo.predict_proba(X_test)[:,1] #Probabilidade de classe positiva (1) (benigno)

# Calcular as métricas para avaliação
precisao = precision_score(Y_test, y_pred) #Precisao = TP / (TP + FP)
recall = recall_score(Y_test, y_pred) #Recall = TP / (TP + FN)
f1 = f1_score(Y_test, y_pred) #F1_Score = 2 * (Precisao * Recall) / (Precisao + Recall)
